...
public class BoardController {
    ...
    @PostMapping
    public ResponseEntity<?> save(@RequestBody @Valid BoardRequest.SaveDTO reqDTO,
            @AuthenticationPrincipal User user) {
        boardService.게시글작성(reqDTO, user.getId());
        return Resp.ok("게시글 작성 완료");
    }

    // 4. 게시글 수정하기 (body가 필요해)
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable("id") Integer id,
            @RequestBody @Valid BoardRequest.UpdateDTO reqDTO,
            @AuthenticationPrincipal User user) {
        boardService.게시글수정(id, reqDTO, user.getId());
        return Resp.ok(null);
    }
